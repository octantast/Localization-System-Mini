...
[МОЖЛИВОСТІ](#можливості) | [ШВИДКИЙ СТАРТ](#швидкий-старт) | [ПІДКАЗКА](#localized-tooltip) | [ФОРМАТ CSV](#формат-csv) | [ШРИФТИ](#керування-шрифтами) | [API](#довідник-api) | [ДЕМО СЦЕНА](#приклад-сцени) | [ПРИМІТКИ](#примітки) | [ЛІЦЕНЗІЯ](#ліцензія) | [ПІДТРИМКА](#підтримка)
...
<a name="localized-tooltip"></a>
## Компонент Localized Tooltip
Компонент для автоматичної локалізації тексту безпосередньо на об'єктах сцени без написання коду.
### Налаштування
1. Додайте компонент **LocalizedTooltip** до GameObject з TextMeshPro
2. Призначте **InputData** у поле Input Text Settings
3. Введіть **Connected Table Key** (ключ рядка з CSV)
4. Текст автоматично оновлюється при Start та зміні мови
### Динамічні Параметри
Для тексту зі змінними використовуйте **Parameter Selector**:
1. Натисніть **+** у списку Dynamic Parameters
2. Виберіть **Target Object** (GameObject з потрібним компонентом)
3. Виберіть **Component** зі спадного списку
4. Виберіть **Field/Property** для відображення
**Оновлення Параметрів:**
- Автоматичне: Розкоментуйте метод `Update()` у LocalizedTooltip.cs
- Ручне: Викликайте `FillTextObject()` після зміни значень
